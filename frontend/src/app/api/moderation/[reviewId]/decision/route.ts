...
export async function POST(
  ...
  try {
    ...
    backendFormData.append('decision', decision);
    if (reviewerId) backendFormData.append('reviewer_id', reviewerId);
    if (notes) backendFormData.append('notes', notes);

    const backendResponse = await fetch(`${BACKEND_URL}/api/moderation/${params.reviewId}/decision`, {
      method: 'POST',
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        { error: errorData.detail || 'Failed to post decision' },
