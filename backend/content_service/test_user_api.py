...
def test_user_listing(persona_name, email, password):
    ...
    try:
        ...
        print(f"   Company: {user.get('company', {}).get('name', 'None')}")
        ...
        if users_response.status_code == 200:
            ...
            print(f"   OK Can access users: {len(users_data)} users returned")
            ...
            print(f"   OK Correctly restricted: {users_response.json().get('detail', 'Access denied')}")
        else:
            print(f"   ❌ Unexpected error: {users_response.status_code}")
...
print(f"\n{'='*50}")
print("OK RBAC User API Testing Complete")
print('='*50)